...
export function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 pt-5 px-4 sm:px-8">
      ...
      {/* Mobile Menu Overlay */}
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden bg-[#060C10]/95 pt-24 px-6 space-y-4">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              onClick={() => setMobileOpen(false)}
              className="block py-3 border-b border-slate-800 text-lg font-bold text-white"
            >
              {link.label}
            </a>
          ))}
          <div className="pt-4">
            <Link href="/login" onClick={() => setMobileOpen(false)}>
              <button className="w-full py-3 rounded-full bg-slate-800 text-white font-bold text-sm">
                Log In
              </button>
            </Link>
          </div>
